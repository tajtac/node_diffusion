...
# A Reimannian Gaussian process regression class (inherits from GPmodel)
# based on https://github.com/spbu-math-cs/Riemannian-Gaussian-Processes/
class ReimannianGP(GPmodel):
    # Initialize the class
    ...
    def compute_cholesky(self, params, batch):
        ...
        K = self.eval_K(X, X, S) + np.eye(N)*(sigma_n + 1e-8)
        L = cholesky(K, lower=True)
        return L
    ...
    def predict(self, X_all, **kwargs):
        ...
        # Compute kernels
        S = self.eval_S(kappa, sigma_f)
        k_pp = self.eval_K(X_all, X_all, S) + np.eye(X_all.shape[0])*(sigma_n + 1e-8)
        k_pX = self.eval_K(X_all, X, S) 
        L = self.compute_cholesky(params, batch)
        alpha = solve_triangular(L.T,solve_triangular(L, y, lower=True))
        beta  = solve_triangular(L.T,solve_triangular(L, k_pX.T, lower=True))
        # Compute predictive mean, std
